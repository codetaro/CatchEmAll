...
class PokeHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    private val pokeCaught: TextView = itemView.findViewById<TextView>(R.id.pokeTextView_caught)

    // called by adapter to instantiate the content with given poke's info
    fun updateWithPoke(poke: Poke) {
        pokeImage.setImageDrawable(poke.imageFile)
